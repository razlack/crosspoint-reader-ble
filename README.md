...
- **Stronger image format handling**: decoder now detects file type by magic bytes (JPEG/PNG) and explicitly rejects unsupported WebP.
- **Download reliability improvements**: adaptive total timeout, no-progress stall timeout, and watchdog resets during long downloads.
- **Network menu expanded**: new "Subreddit Reader" option alongside Join Network, Calibre Wireless, and Hotspot modes.
